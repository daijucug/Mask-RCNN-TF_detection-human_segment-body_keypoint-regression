...
def loadData(image,instance_mask,parts_mask,keypoints):
    gt_boxes = []
    masks_instances = []
    _,contours,hierarchy = cv2.findContours(instance_mask.copy(), 1, 2)
    x1=100000
    y1=100000
    x2=-10000
    y2=-10000
    ...
    for x in range(1,11):
        ...
        masks_for_person[...,body_parts_dict[x]] = np.logical_or(masks_for_person[...,body_parts_dict[x]],part)

    for x in range(15):
        # keypoints[0,x] = keypoints[0,x]-x1
        # keypoints[1,x] = keypoints[1,x]-y1
        keypoints[0,x] = map_value(keypoints[0,x],x1,x2,0.0,112.0)
        keypoints[1,x] = map_value(keypoints[1,x],y1,y2,0.0,112.0)
        keypoints[0,x] = map_value(keypoints[0,x],0.0,112.0,-1,1)
...
with tf.python_io.TFRecordWriter(record_filename, options=options) as tfrecord_writer:
    for s in scenes:
        ...
        for mask in mask_dir:
            ...
            mat_file_parts = sio.loadmat('puppet_flow_ann/'+s+'/'+mask+'/puppet_flow.mat')
