...
for record in tf.python_io.tf_record_iterator('out_human_and_body_parts_keypoints_JHMDB.tfrecord',options):
            ...
            for x in range(int(h_box[0]),int(h_box[2])):
                for y in range(int(h_box[1]),int(h_box[3])):
                    if mask[y,x]==1:
                        ...
                        hsv[y,x,1] = S
        for x in range(15):
            gt_keypointsnp[0,x] = map_value(gt_keypointsnp[0,x],-1.0,1.0,h_box[0],h_box[2])
            gt_keypointsnp[1,x] = map_value(gt_keypointsnp[1,x],-1.0,1.0,h_box[1],h_box[3])
