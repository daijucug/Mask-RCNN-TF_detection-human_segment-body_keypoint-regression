...
def draw_human_body_parts(step, image, name='', image_height=1, image_width=1, bbox=None, label=None, gt_label=None, prob=None,final_mask=None,keyp=None,gt_keyp=None):
    import cv2
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    if bbox is not None:
        dictinary = {}
        for i, box in enumerate(bbox):
            ...
            if (prob[i,label[i]] > 0.5) and width*height >1000 and label[i]!=0:
                ...
                for x in range(1,7):
                    ...
                    masks[...,x] = maska
                dictinary[round(area,4)]=(box,label[i],gt_label[i],prob[i,label[i]],masks,colors[label[i]],keyp[i],gt_keyp[i])
        sorted_keys = sorted(dictinary.iterkeys(),reverse=True)

        for key,i in zip(sorted_keys,range(len(sorted_keys))):
            bo, lab,gt_lab,_,mask,col,keyp,gt_keyp= dictinary[key]
            keyp = keyp.reshape((2,15))
            print mask.shape
            ...
            for x in range(int(bo[0]),int(bo[2])):
                for y in range(int(bo[1]),int(bo[3])):
                    ...
                    if mask[ym,xm,max_indices[ym,xm]] >0:
                        ...
                        hsv[y,x,1] = 255
            for x in range(15):
                keyp[0,x] = map_value(keyp[0,x],-10.0,10.0,bo[0],bo[2])
                keyp[1,x] = map_value(keyp[1,x],-10.0,10.0,bo[1],bo[3])
                hsv = cv2.circle(hsv,(int(keyp[0,x]),int(keyp[1,x])),2,(255,255,255))

        hsv = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)
        i=0
        for key in sorted_keys:
            bo, lab,gt_lab,_,_,col,_,_= dictinary[key]
            if lab != gt_lab:
                c = (0,0,255)
            else:
                c = (255,0,0)
            ...
            hsv = cv2.putText(hsv,text+' '+str(i),(2+int(bo[0]),2+int(bo[1])), cv2.FONT_HERSHEY_SIMPLEX,0.5, color =(255,255,255))
    #cv2.imwrite('test_' + name + '_' +  str(step) +'.jpg',image)
    cv2.imwrite('/home/alex/PycharmProjects/data_keypoints/test' + name + '_' +  str(step) +'.jpg',hsv)
