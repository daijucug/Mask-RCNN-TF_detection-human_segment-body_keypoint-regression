...
for x in range(0,512):
    ...
    print (x)
    image= draw_human_body_parts(1,image,name="seg"+str(x),bbox=bbox,label=label,gt_label=gt_label,prob=prob,final_mask=final_mask)

    cv2.imwrite('output_seg/test' + "seg"+str(x) + '_' +  str(x) +'.jpg',image)
