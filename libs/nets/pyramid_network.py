...
def _filter_negative_samples(labels, tensors):
    ...
    filtered = []
    for t in tensors:
        with tf.control_dependencies([tf.assert_equal(tf.shape(t)[0], tf.shape(labels)[0])]):
            f = tf.gather(t, keeps)
            filtered.append(f)
...
def build_pyramid(net_name, end_points, bilinear=True):
  ...
  if isinstance(net_name, str):
    pyramid_map = _networks_map[net_name] #net_name is resnet50 so it returns a dictionary with the names of the layers
  else:
    pyramid_map = net_name #net_name is  a dictionary with the names of the layers
...
def build_heads(pyramid, ih, iw, num_classes, base_anchors, is_training=False, gt_boxes=None):
  ...
  with slim.arg_scope(arg_scope):
    with tf.variable_scope('pyramid'):
        ...
        # to 28 x 28
        #average_m = tf.reduce_mean(m,axis=3)
        ...
        kp = cropped_rois
        for _ in range(2):
            kp = slim.conv2d(kp, 64, [3, 3], stride=1, padding='SAME', activation_fn=tf.nn.relu)
        average_kp = slim.conv2d(m, 1, [1, 1], stride=1, padding='SAME', activation_fn=tf.nn.relu)
        keypoints = slim.flatten(average_kp)
        keypoints = slim.fully_connected(keypoints, 392, activation_fn=tf.nn.tanh,weights_initializer=tf.truncated_normal_initializer(stddev=0.05))
        keypoints = slim.fully_connected(keypoints, 2*15, activation_fn=None,weights_initializer=tf.truncated_normal_initializer(stddev=0.05))
        outputs['keypoints'] = keypoints
        outputs['final_keypoints'] = tf.reshape(keypoints, (2, 15))

        number_of_parts = 7
        m = slim.conv2d(m, number_of_parts, [1, 1], stride=1, padding='VALID', activation_fn=None)
        #adaugat de mine:
        final_mask_for_drawing = tf.sigmoid(m,"mask_sigmoid")
        # add a mask, given the predicted boxes and classes
        outputs['mask'] = {'mask':m, 'cls': classes, 'score': scores,'final_mask_for_drawing':final_mask_for_drawing}
          
  return outputs

def loss(logits, labels, num_classes, head=None): #this is softmax cross entropy loss. I did this like here because softmax_cross_entropy_loss was throwing errors
...
def build_losses(pyramid, outputs, gt_boxes, gt_masks,gt_keypoints,
  ...
  *_lw: loss weight of rpn, refined and mask losses
  ...
  with slim.arg_scope(arg_scope):
      with tf.variable_scope('pyramid'):
        ...
        if FLAGS.INCLUDE_MASK==True:
            tf.add_to_collection(tf.GraphKeys.LOSSES, mask_loss)
            mask_losses.append(mask_loss)

        keypoints = outputs['keypoints']
        keypoints_loss = 0.05 * tf.reduce_mean(tf.reduce_sum(tf.square(keypoints - gt_keypoints),axis=1))
        tf.add_to_collection(tf.GraphKeys.LOSSES, keypoints_loss)
        #0.01 is the weight loss
  ...
  if FLAGS.INCLUDE_MASK==True:
    ...
    losses = [rpn_box_losses, rpn_cls_losses, refined_box_losses, refined_cls_losses, mask_losses,keypoints_loss]
  else:
...
def build(end_points, image_height, image_width, pyramid_map, 
    ...
    if is_training:
        loss, losses = build_losses(pyramid, outputs,
                        gt_boxes, gt_masks,gt_keypoints,
