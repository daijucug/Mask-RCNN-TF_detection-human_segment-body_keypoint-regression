...
def preprocess_for_training(image, gt_boxes, gt_masks,gt_keypoints):
            ...
            tf.cond(tf.greater_equal(coin, 0.5), 
                    lambda: (preprocess_utils.flip_image(image),
                            ...
                            preprocess_utils.flip_gt_masks(gt_masks),
                             preprocess_utils.flip_gt_keypoints(gt_keypoints)),
                    lambda: (image, gt_boxes, gt_masks))

    ## min size resizing
    new_ih, new_iw = preprocess_utils._smallest_size_at_least(ih, iw, cfg.FLAGS.image_min_size)#FLAGS.image_min_size = 640(min edge)
