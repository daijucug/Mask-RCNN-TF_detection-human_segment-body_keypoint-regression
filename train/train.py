...
def save(step,input_imagenp,final_boxnp,gt_boxesnp,final_clsnp,final_probnp,final_gt_clsnp,final_masknp=None,gt_masksnp=None,keypointsnp=None,gt_keypointsnp=None):
    save_array = []
    ...
    save_array.append(keypointsnp)
    save_array.append(gt_keypointsnp)
    save_array = np.asarray(save_array)
    np.save("draw/array"+str(step)+".npy",save_array)

def train():
    """The main function that runs training"""

    ## data
    image, ih, iw, gt_boxes, gt_masks, num_instances, img_id,gt_keypoints = \
    ...
    outputs = pyramid_network.build(end_points, im_shape[1], im_shape[2], pyramid_map,
            ...
            base_anchors=9,
            is_training=True,
            gt_boxes=gt_boxes, gt_masks=gt_masks,gt_keypoints=gt_keypoints,
    ...
    for step in range(FLAGS.max_iters):
        ...
        if FLAGS.INCLUDE_MASK:
            s_, tot_loss, reg_lossnp, img_id_str, rpn_box_loss, rpn_cls_loss, refined_box_loss, refined_cls_loss,mask_loss,keypoints_loss, gt_boxesnp, input_imagenp, final_boxnp, final_clsnp, final_probnp, final_gt_clsnp, gtnp, tmp_0np, tmp_1np, tmp_2np, tmp_3np, tmp_4np, final_masknp,gt_masksnp ,keypointsnp,gt_keypointsnp= sess.run([update_op, total_loss, regular_loss, img_id] + losses + [gt_boxes] + [input_image] + [final_box] + [final_cls] + [final_prob] + [final_gt_cls] + [gt] + [tmp_0] + [tmp_1] + [tmp_2] + [tmp_3] + [tmp_4]+[final_mask]+[gt_masks]+[keypoints]+[gt_keypoints])
        else:
        ...
        # print (keypointsnp)
        # print (gt_keypointsnp)

        duration_time = time.time() - start_time
        if step % 1 == 0:
            if FLAGS.INCLUDE_MASK:
                print ( """iter %d: image-id:%07d, time:%.3f(sec), regular_loss: %.9f, """
                        """total-loss %.10f(%.4f, %.4f, %.6f, %.4f,%.5f, %.5f), """ #%.4f
                        """instances: %d, proposals: %d """
                       % (step, img_id_str, duration_time, reg_lossnp,
                          tot_loss, rpn_box_loss, rpn_cls_loss, refined_box_loss, refined_cls_loss, mask_loss,keypoints_loss,
            ...
            if sys.argv[1]=='--draw':
                if FLAGS.INCLUDE_MASK:
                    save(step,input_imagenp,final_boxnp,gt_boxesnp,final_clsnp,final_probnp,final_gt_clsnp,final_masknp,gt_masksnp,keypointsnp,gt_keypointsnp)
                else:
